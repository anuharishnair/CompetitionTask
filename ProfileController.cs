...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        public async Task<IActionResult> UpdateProfilePhoto(string talentId, IFormFile file)
        {
            ...
            try
            {
                // Call the service to update the profile photo
                var result = await _profileService.UpdateTalentPhoto(talentId, file);
                var profile = (await _userRepository.Get(x => x.Id == talentId)).SingleOrDefault();
                if (result)
                {
                    return Ok(new { profilePhotoUrl = profile?.ProfilePhotoUrl });
                }
                else
                {
            ...
            {
                _logger.LogError(ex, "An error occurred while processing the profile photo update.");
